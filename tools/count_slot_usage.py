...
MEETINGS_PATH = os.path.abspath('./meetings/')
...
def main():
    ...
    for hour in range(24):
        counts[hour] = {k: [] for k in WEEKDAYS}
    ...
    for day in sorted(WEEKDAYS, key=lambda key: WEEKDAYS[key]):
        for hour in range(24):
            if len(meeting_counts[hour][day]) >= full_time_slot:
    ...
    out_fname = os.path.abspath('busy.csv')
    write_csv_file(out_fname, meeting_counts)
...
def calculate_meeting_counts(meetings):
    ...
    for meeting in meetings:
        ...
        for schedule in meeting['schedule']:
            ...
            meeting_info = (
                "{:<13} - {:<21} - {}".format(frequency, irc, meeting_id))
